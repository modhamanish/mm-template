    ...
    render() {
        ...
        return (
            <View style={{ zIndex: 999 }}>
                ...
                <View style={{ zIndex: 999 }}>
                    <FlatList
                        contentContainerStyle={{ zIndex: 1000, width: '100%', backgroundColor: '#fff', }}
                        style={{ position: 'absolute', width: '100%' }}
                        data={result}
                        renderItem={({ item, index }) =>
                            <TouchableOpacity activeOpacity={.9} style={{ zIndex: 1111, paddingVertical: 8, borderBottomWidth: 1, }} onPress={() => this.setState({ result: [] }, () => onChangeText(item))}>
                                <Text>{item}</Text>
                            </TouchableOpacity>
                        }
                    />
